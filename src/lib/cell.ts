import { ReadonlyCollection } from "./types";
import { Style } from "./style";
import { Border } from "./border";
import { TableRowGroup } from "./rowGroup";
import { TableColumn } from "./column";
import { TableRow } from "./row";
import { Table } from "./table";
import { measureText } from "./text";
import { maxInt32, minMax } from "./utils";

export class TableCell<T> {
    readonly table: Table<T>;
    readonly group: TableRowGroup<T> | undefined;
    readonly row: TableRow<T>;
    readonly column: TableColumn<T> | undefined;
    readonly columnSpan: number;
    readonly rowSpan: number;
    readonly text: string;

    /*@internal*/ _actualHeight: number = -1;
    /*@internal*/ _actualWidth: number = -1;
    /*@internal*/ _actualStyle: Style | undefined;
    /*@internal*/ _lines: string[] | undefined;

    constructor(table: Table<T>, group: TableRowGroup<T> | undefined, row: TableRow<T>, column: TableColumn<T> | undefined, columnSpan: number, text: string) {
        this.table = table;
        this.group = group;
        this.row = row;
        this.column = column;
        this.columnSpan = Math.max(1, columnSpan);
        this.rowSpan = 1;
        this.text = text;
        const desiredWidth = measureText(this.text, maxInt32).width;
        if (column) {
            if (column._desiredWidth < desiredWidth) {
                column._desiredWidth = desiredWidth;
            }
        }
    }
}

